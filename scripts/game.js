(function gameSetup() {
    ...
    var viewportWidth = window.innerWidth ; // for screen wrapping
    var viewportHeight = window.innerHeight; // for screen wrapping
    ...
    // set shipsCurrentAngle & shipsCurrentVelocity to ship angle & ship velocity
    let shipsCurrentAngle = ship.angle;
    let shipsCurrentVelocity = ship.velocity;

    // function to change the ships color based on velocity
    ...
    function countdown() {
        ...
        function tick() {
            seconds--;
            p.innerHTML = "0:" + (seconds < 10 ? "0" : "") + String(seconds); //ternary operator shortcut for if statment
            if( seconds > 0 ) {
